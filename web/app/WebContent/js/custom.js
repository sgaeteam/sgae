jQuery(document).ready(function(){
	...
	$(function(){
	    ...
	    $("#cpf1").keyup(function(){
	       ...
	       if(tamanho >= limite){
	    	   $.getJSON('../sgae/usuarios/'+cpf, function (json) {
	    		   var html = '';
	    		   for($i=0; $i < json.unidade.length; $i++){
	                   html += '<option title="'+json.unidade[$i].nome+'" value="'+json.unidade[$i].id +'">'+json.unidade[$i].nome+'</option>';
	               }
	    		   if(html == ''){
	    			   $.msgbox("O C.P.F informado n\u00E3o foi localizado.", {type: "info",buttons : [{type: "submit", value: "Sim"}]});
	    			   $("#unidade").hide();
	    	    	   $("#senha").hide();
	    		   } else {
	    			 $("#unidade").show();
...
$('#bt_entrar').click(function(ev) {
	if($('#cpf1').val() == "" || $('#unidades option').size() == 0 || $('#password').val() == ""){
     $.msgbox("Por favor preencha todos os campos do formulario!", {type: "info",buttons : [{type: "submit", value: "Sim"}]});
     return false;
	}
});
...
$('.link_aluno_delete').click(function(ev) {
   var url = $(this).attr("href");
   $.msgbox("Tem certeza de que deseja excluir permanentemente o elemento selecionado?", {
	type: "confirm",
	buttons : [
        {type: "submit", value: "Sim"},{type: "submit", value: "N\u00E3o"}]},
	function(result) { 
      if (result == 'Sim')
	  {				
...
$(document).ready( function() {// When the Dom is ready
    $(".error").fadeIn().delay(3000).fadeOut(); //Add a fade out effect that will last for 2000 millisecond
    $(".alert").fadeIn().delay(3000).fadeOut();
    $(".notice").fadeIn().delay(3000).fadeOut();
    $(".success").fadeIn().delay(3000).fadeOut();
    $(".info").fadeIn().delay(3000).fadeOut();	
});
...
/**
 * Modais dos formulários de aluno 
 */

$('#botao_aluno_new').click(function (e) {
	$('#form_aluno_new').modal(); 
	return false;
});

$('.link_aluno_edit').click(function (e) {
	var form = '#form_aluno_edit_'+$(this).attr("id");
	$(form).modal(); 
	return false;
});

$('.link_aluno_view').click(function (e) {
	var form = '#form_aluno_view_'+$(this).attr("id");
	$(form).modal(); 
	return false;
});
...
/**
 * Jquery para abrir o calendário popup.
 */
    
$('.calendario-popup').focusin(function (e) {
